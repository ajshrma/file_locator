...
var File = require('../models/file.schema');
...
/* 
	Send files related to the employee
*/
employeeFileRouter
	.route('/')
	.get((req, res, next) => {
...
employeeFileRouter
	...
	.post((req, res, next) => {
		/* 
			TODO
			1. Add a check to add files once an employee at any step, if file already alloted then send 409
			2. Send email to the qr generator on file allotment success
		*/
		/* Users model - check if file already exists for in that users array, return 409 */

		File.findById(req.params.fileId)
			.lean()
			.then(
				file => {
					const fileAlreadyAssigned = file.steps
						.map(step => {
							if (!step.status && step.empID) {
								const err = new Error('File already assigned');
								err.status = 403;
								next(err);
								return;
							}
